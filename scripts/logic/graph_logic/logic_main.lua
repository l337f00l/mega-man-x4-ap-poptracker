...
mega_man_x4_location = {}
mega_man_x4_location.__index = mega_man_x4_location
...
function mega_man_x4_location.new(name)
    local self = setmetatable({}, mega_man_x4_location)
    if name then
...
function mega_man_x4_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = mega_man_x4_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function mega_man_x4_location:connect_two_ways(exit, rule)
...
function mega_man_x4_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function mega_man_x4_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function mega_man_x4_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function mega_man_x4_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function mega_man_x4_location:accessibility()
...
-- 
function mega_man_x4_location:discover(accessibility, keys)
...
entry_point = mega_man_x4_location.new("entry_point")
